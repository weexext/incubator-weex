...
//        _titleColor = [UIColor colorWithRed:56/255.f green:81/255.f blue:152/255.f alpha:1.f];
    ...
    if ([self.ucx_param count]>0) {
        ...
        if ([ucx_navBar count]>0 && [ucx_navBar isKindOfClass:[NSDictionary class]]) {
            ...
            NSNumber *navBarHeight = [ucx_navBar objectForKey:@"height"];
            if (navBarHeight) {
                ucx_navBarHeight = [navBarHeight floatValue];
            }
    ...
    self.naviView = [[UIView alloc] initWithFrame:CGRectMake(0, 0, width, ucx_navBarHeight*width/750)];
